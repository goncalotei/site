...
class MDNLayer(nn.Module):
    def __init__(self, input_dim, action_dim, k=1):
        ...
        # ── σ como parâmetro por-expert (independente do input) ──────────────
        # Com features=0 (step==0, bin vazio), sigma_layer linear daria
        # sigma = bias = mesmo valor para todos os experts.
        # Aqui cada expert tem o seu próprio log_sigma aprendido de forma
        # independente: sigma_k pode ser diferente de sigma_j.
        # Inicializado a log(0.02) ≈ -3.91 → σ≈0.02 por dimensão.
        # L_SIGMA vai empurrar o winner para o piso MIN_SIGMA.
        MIN_SIGMA = 0.005
        MAX_SIGMA = 0.08
        init_sigma = 0.02
        self.log_sigma = nn.Parameter(
            torch.full((k, action_dim), math.log(init_sigma))
        )
    ...
    def forward(self, x, gate_temp=1.0):
        # σ: por-expert, partilhado entre todas as amostras do mesmo batch.
        # clamp em [MIN_SIGMA, MAX_SIGMA] para impedir colapso ou difusão excessiva.
        sigma = torch.clamp(
            torch.exp(self.log_sigma), min=self.MIN_SIGMA, max=self.MAX_SIGMA
        ).unsqueeze(0).expand(x.size(0), -1, -1)   # [B, K, D]
...
for epoch in range(EPOCHS):
    ...
    if (epoch + 1) % 10 == 0:
        ...
        with torch.no_grad():
            ...
            # Sigma por expert (parâmetro livre — pode ser diferente entre experts)
            with torch.no_grad():
                sig_per_expert = torch.clamp(
                    torch.exp(model.mdn.log_sigma),
                    min=model.mdn.MIN_SIGMA, max=model.mdn.MAX_SIGMA
                ).cpu().numpy()   # [K, D]
            sig_str = " ".join(
