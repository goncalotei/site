...
SEED             = 42
L_GATE           = 0.0   
L_SIGMA          = 0.30    # forçar sigma pequeno no winner — forte porque σ é parâmetro livre
PATIENCE         = 800     # épocas sem melhoria para early stopping
...
def mdn_loss_packing(pi, sigma, mu, target, l_sep=0.0, l_sigma=0.0, l_gate=0.0):
    # ------------------------------------------------------------
    # 1. Mixture Negative Log-Likelihood (core MDN objective)
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # 2. Posterior responsibility (E-step style signal)
    #    r_k = P(expert k | x, y)
    # ------------------------------------------------------------
    r = torch.softmax(log_joint, dim=1).detach()
    # ------------------------------------------------------------
    # 3. Gate regularization 
    #    Aligns gating network with true posterior responsibility
    #    -> automatically adapts to uni or multi modality
    # ------------------------------------------------------------
    gate_kl = torch.sum(r * (torch.log(r + 1e-8) - torch.log(pi + 1e-8)), dim=1).mean()
    # ------------------------------------------------------------
    # 4. Separation loss (prevents expert collapse)
    # ------------------------------------------------------------
    mu_xyz  = mu[:, :, :3]
    ...
    # ------------------------------------------------------------
    # 5. Sigma regularization (avoid high uncertainty in winner)
    # ------------------------------------------------------------
...
class MDNLayer(nn.Module):
    def __init__(self, input_dim, action_dim, k=1):
        ...
        # ── σ dependente do input (heteroscedástico) ─────────────────────────
        # sigma_layer: [B, K*D] → reshape [B, K, D]
        # Com features=0 (step=0), σ = bias por expert → cada expert tem bias
        # independente → experts podem ter σ diferente.
        # Quando houver step>0, o modelo aprende σ adaptativo ao estado do bin.
        # Bounds: softplus garante σ>0; clamp [0.005, 0.10] = [0.17cm, 3.46cm] em X.
        self.sigma_layer = nn.Linear(input_dim, k * action_dim)
        # Inicializar bias para σ_init ≈ 0.02 → softplus⁻¹(0.02) ≈ log(e^0.02 - 1)
        with torch.no_grad():
            nn.init.constant_(self.sigma_layer.bias,
                               math.log(math.exp(0.02) - 1.0))  # softplus⁻¹(0.02)
            nn.init.zeros_(self.sigma_layer.weight)              # início: só bias conta

        with torch.no_grad():
            # sigmoid(logit(p)) = p  →  inicializa experts espalhados pela caixa
            xs_logit = torch.logit(torch.linspace(0.1, 0.9, k))
            ys_logit = torch.logit(torch.linspace(0.1, 0.9, k))
            for i in range(k):
                base = i * action_dim
                self.mu.bias.data[base]     = xs_logit[i]   # x_norm via sigmoid
                self.mu.bias.data[base + 1] = ys_logit[i]   # y_norm via sigmoid
                self.mu.bias.data[base + 2] = 0.2            # z_norm livre
                self.mu.bias.data[base + 3] = 0.0            # sin_yaw
                self.mu.bias.data[base + 4] = 1.0            # cos_yaw
            nn.init.constant_(self.pi_logits.bias, 0.0)
            nn.init.constant_(self.pi_logits.weight, 0.0)

    def forward(self, x, gate_temp=1.0):
        # σ dependente do input: softplus garante >0, clamp limita [0.005, 0.10]
        sigma = torch.clamp(
            F.softplus(self.sigma_layer(x)), min=0.005, max=0.05
        ).view(-1, self.k, self.action_dim)         # [B, K, D]

        mu_raw = self.mu(x).view(-1, self.k, self.action_dim)
        # μ_x e μ_y via sigmoid → sempre dentro de [0,1] normalizado (dentro da caixa)
        mu_xy   = torch.sigmoid(mu_raw[:, :, :2])
        mu_rest = mu_raw[:, :, 2:]
        mu      = torch.cat([mu_xy, mu_rest], dim=2)
...
# MAIN — TREINO
# ==========================================
# 4 critérios de seleção — guardados em paralelo durante o treino
best_nll              = float('inf');  best_nll_weights        = None   # menor NLL pura
best_resp_dist        = float('inf');  best_resp_weights       = None   # menor RespDist mediana
best_resp_mean        = float('inf');  best_resp_mean_weights  = None   # menor RespDist média
best_resp90           = float('inf');  best_resp90_weights     = None   # menor RespDist 90% trimmed
best_avgdist_mean     = float('inf');  best_avgdist_mean_weights   = None  # menor AvgDist média
best_avgdist_median   = float('inf');  best_avgdist_median_weights = None  # menor AvgDist mediana
best_avgdist90        = float('inf');  best_avgdist90_weights      = None  # menor AvgDist 90% trimmed
best_combined90       = float('inf');  best_combined90_weights = None   # menor RespD90+AvgDist90
_es_counter    = 0
_es_best       = float('inf')
...
for epoch in range(EPOCHS):
    ...
    if (epoch + 1) % 1 == 0:
        ...
        with torch.no_grad():
            ...
            # ── Uma passagem: RespDist + NLL pura + AvgDist argmax ──────────
            ...
            for h, p, a in ref_loader:
                ...
                pi, sig, mu = model(h, p, gate_temp=0.3)
                B = h.size(0)
                last_sig = sig  # guardar para sig_str (features=0 → igual em todos os batches)
                # RespDist
                rd = compute_responsibility_dist(pi, sig, mu, a, scales)
                all_rd.extend((rd * 100).cpu().numpy().tolist())
                # NLL pura (sem regularização)
            ...
            # Sigma por expert — extraído do último batch (features=0 → igual em todos)
            sig_per_expert = last_sig[0].detach().cpu().numpy()   # [K, D]
            sig_str = " ".join(
            ...
            log_print(
                ...
                f"NLL:{nll_score:.3f}[B:{best_nll:.3f}] "
                f"Dist90:{avg_dist_90:.2f}[B:{best_avgdist90:.2f}] "
                f"Cmb90:{combined_90:.2f}[B:{best_combined90:.2f}]cm | "
...
# Avaliação detalhada usa o modelo selecionado por RespDist
best_weights = best_resp_weights
model.load_state_dict(best_weights)

base = f"model_cracker_box_k{K}_seed{SEED}"
_save_info = [
    ("resp",       best_resp_weights,        f"RespDist(median)={best_resp_dist:.2f}cm"),
...
log_print(f"   ℹ️  Avaliação detalhada abaixo usa modelo 'resp'")
...
if history['val']:
    axes[0].plot(history['val'], label='Val', color='C1')

if history['test']:
    axes[0].plot(history['test'], label='Test', color='green')
...
axes[1].set_title('Responsibility Dist')
...
# ==========================================
# PLOT — COMPARAÇÃO DOS 4 CRITÉRIOS DE SELEÇÃO
# ==========================================
def _load_and_get_experts(weights, ref_step, ref_dataset, device):
    """Carrega pesos no modelo e faz um forward pass para obter distribuição dos experts."""
...
# GT de todos os samples de treino
all_gts_xy = np.array([[s['human_action'][0], s['human_action'][1]] for s in train_d])

_criteria = [
    ("RespDist",   best_resp_weights,       f"Best={best_resp_dist:.2f}cm"),
    ("NLL",        best_nll_weights,        f"Best={best_nll:.4f}"),
    ("AvgDist90",  best_avgdist90_weights,  f"Best={best_avgdist90:.2f}cm"),
    ("Combined90", best_combined90_weights, f"Best={best_combined90:.2f}cm"),
]
...
for ax, (criterion, weights, score_str) in zip(axes, _criteria):
    if weights is None:
        ax.set_title(f'{criterion}\n(sem dados)')
        ax.axis('off')
        continue

    pi_v, sig_v, mu_v = _load_and_get_experts(weights, _ref_step, _ref_dataset, device)

    # GT como pontos de referência
    ax.scatter(all_gts_xy[:, 0], all_gts_xy[:, 1],
               c='black', marker='+', s=70, alpha=0.45, label='GT', zorder=1)

    # Expert distributions
    for i in range(K):
        mx = mu_v[i, 0] * 0.345987
        my = mu_v[i, 1] * 0.227554
        sx = sig_v[i, 0] * 0.345987
        sy = sig_v[i, 1] * 0.227554
        theta = np.linspace(0, 2 * np.pi, 120)
        # 1σ e 2σ ellipses
        ax.plot(mx + sx * np.cos(theta), my + sy * np.sin(theta),
                color=colors_k[i], alpha=0.75, linewidth=1.8)
        ax.plot(mx + 2*sx * np.cos(theta), my + 2*sy * np.sin(theta),
                color=colors_k[i], alpha=0.25, linewidth=1.0, linestyle='--')
        ax.scatter(mx, my, color=colors_k[i], s=220, marker='*', zorder=5,
                   label=f'E{i+1} π={pi_v[i]*100:.0f}%  σ=({sx*100:.1f},{sy*100:.1f})cm')

    # Bordo da caixa
    th = 0.005
    ax.add_patch(Rectangle((-th/2, -th/2), 0.346+th, 0.2275+th,
                            linewidth=3, edgecolor='red', facecolor='none', zorder=2))
    ax.set_xlim(-0.05, 0.40); ax.set_ylim(-0.025, 0.30)
    ax.set_xlabel('X (m)'); ax.set_ylabel('Y (m)')
    ax.set_title(f'{criterion}\n{score_str}', fontsize=11, fontweight='bold')
    ax.legend(fontsize=7, loc='upper right', framealpha=0.8)
    ax.grid(True, alpha=0.3)

plt.suptitle(
    f'{KEYWORD} — Comparação de critérios de seleção do modelo\n'
    f'(elipses: 1σ sólido · 2σ tracejado | ★ = μ expert)',
    fontsize=12
)
plt.tight_layout()
plt.savefig('model_selection_comparison.png', bbox_inches='tight', dpi=150)
plt.show()
log_print("✅ Plot guardado: model_selection_comparison.png")

# Restaurar o modelo 'resp' para consistência
model.load_state_dict(best_weights)
# ==========================================
# PLOT — Evolution of distance metrics
# ==========================================
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

ax = axes[0]
ax.plot(history['epochs'], history['avgdist_mean'],   label='AvgDist Mean',   color='C0')
ax.plot(history['epochs'], history['avgdist_median'], label='AvgDist Median', color='C1')
ax.plot(history['epochs'], history['avgdist90'],      label='AvgDist 90%',     color='C2')

ax.axhline(best_avgdist_mean,   color='C0', linestyle='--', alpha=0.5,
           label=f'Best mean={best_avgdist_mean:.2f}cm')
ax.axhline(best_avgdist_median, color='C1', linestyle='--', alpha=0.5,
           label=f'Best median={best_avgdist_median:.2f}cm')
ax.axhline(best_avgdist90,      color='C2', linestyle='--', alpha=0.5,
           label=f'Best 90%={best_avgdist90:.2f}cm')

ax.set_xlabel('Epoch')
ax.set_ylabel('cm')
ax.set_title('AvgDist — evolution by criterion')
ax.legend(fontsize=7)
ax.grid(True, alpha=0.3)

ax = axes[1]
ax.plot(history['epochs'], history['resp_mean'],  label='RespDist Mean',   color='C3')
ax.plot(history['epochs'], history['resp_dist'],  label='RespDist Median', color='C4')
ax.plot(history['epochs'], history['resp90'],     label='RespDist 90%',     color='C5')

ax.axhline(best_resp_mean,  color='C3', linestyle='--', alpha=0.5,
           label=f'Best mean={best_resp_mean:.2f}cm')
ax.axhline(best_resp_dist,  color='C4', linestyle='--', alpha=0.5,
           label=f'Best median={best_resp_dist:.2f}cm')
ax.axhline(best_resp90,     color='C5', linestyle='--', alpha=0.5,
           label=f'Best 90%={best_resp90:.2f}cm')

ax.set_xlabel('Epoch')
ax.set_ylabel('cm')
ax.set_title('RespDist — evolution by criterion')
ax.legend(fontsize=7)
ax.grid(True, alpha=0.3)

plt.suptitle(f'{KEYWORD} — Training Metrics Over Time', fontsize=12)
plt.tight_layout()
plt.savefig('metrics_evolution.png', bbox_inches='tight', dpi=150)
plt.show()

log_print("✅ Plot saved: metrics_evolution.png")

# ==========================================
# HELPER — plot expert distribution num eixo
# ==========================================
def _plot_expert_panel(ax, weights, criterion, score_str, all_gts_xy,
                       ref_step, ref_dataset, device):
    """Plota distribuição dos experts + GT num único eixo matplotlib."""
    if weights is None:
...
# ==========================================
# PLOT — AvgDist: mean · median · 90% trimmed
# ==========================================
fig, axes = plt.subplots(1, 3, figsize=(18, 6))

_avgdist_criteria = [
    ("AvgDist — Mean",      best_avgdist_mean_weights,   f"Best mean={best_avgdist_mean:.2f}cm"),
    ("AvgDist — Median",    best_avgdist_median_weights, f"Best median={best_avgdist_median:.2f}cm"),
    ("AvgDist — 90% Trim",  best_avgdist90_weights,      f"Best avg90={best_avgdist90:.2f}cm"),
]
...
plt.suptitle(
    f'{KEYWORD} — Model Selection by AvgDist (Argmax)\n'
    f'(ellipses: 1σ solid · 2σ dashed | ★ = μ expert)',
    fontsize=12
)
...
log_print("✅ Plot saved: model_selection_avgdist.png")
# ==========================================
# PLOT — RespDist: mean · median · 90% trimmed
# ==========================================
fig, axes = plt.subplots(1, 3, figsize=(18, 6))

_resp_criteria = [
    ("RespDist — Mean",     best_resp_mean_weights, f"Best mean={best_resp_mean:.2f}cm"),
    ("RespDist — Median",   best_resp_weights,      f"Best median={best_resp_dist:.2f}cm"),
    ("RespDist — 90% Trim", best_resp90_weights,    f"Best avg90={best_resp90:.2f}cm"),
]
...
plt.suptitle(
    f'{KEYWORD} — Model Selection by RespDist\n'
    f'(ellipses: 1σ solid · 2σ dashed | ★ = μ expert)',
    fontsize=12
)
...
log_print("✅ Plot saved: model_selection_respdist.png")
