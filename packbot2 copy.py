...
L_GATE          = 0.0
L_SIGMA         = 0.30
PATIENCE        = 250          # épocas sem melhoria → early stopping
ONLY_FIRST_STEP = True

# Seeds para testar — o melhor é guardado automaticamente
SEEDS = [42, 132, 777, 2024, 9999]
...
# Critérios globais — cada um acumula o melhor de TODOS os seeds+epochs
# {valor, weights, seed que o produziu}
gb = {
    'nll':            {'val': float('inf'), 'w': None, 'seed': None},
    'resp_dist':      {'val': float('inf'), 'w': None, 'seed': None},
    'resp_mean':      {'val': float('inf'), 'w': None, 'seed': None},
    'resp90':         {'val': float('inf'), 'w': None, 'seed': None},
    'avgdist_mean':   {'val': float('inf'), 'w': None, 'seed': None},
    'avgdist_median': {'val': float('inf'), 'w': None, 'seed': None},
    'avgdist90':      {'val': float('inf'), 'w': None, 'seed': None},
    'combined90':     {'val': float('inf'), 'w': None, 'seed': None},
}
global_best_history    = None   # history do seed com melhor resp_dist
...
for seed_run, SEED in enumerate(SEEDS):
    ...
    # ── Rastreadores POR SEED (reset a cada seed) ──────────────────
    _es_counter = 0
    _es_best    = float('inf')
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            ...
            # ── Atualizar critérios globais (cross-seed) ──────────
            snap = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
            metrics = {
                'nll':            nll_score,
                'resp_dist':      median_dist,
                'resp_mean':      resp_mean_full,
                'resp90':         resp_mean_90,
                'avgdist_mean':   avg_dist_mean,
                'avgdist_median': avg_dist_median,
                'avgdist90':      avg_dist_90,
                'combined90':     combined_90,
            }
            for key, val in metrics.items():
                if val < gb[key]['val']:
                    gb[key] = {'val': val, 'w': snap, 'seed': SEED}

            # ── Early stopping por seed ───────────────────────────
            ...
            if (epoch + 1) % 10 == 0:
                ...
                log_print(
                    ...
                    f"RespD:{median_dist:.2f}[G:{gb['resp_dist']['val']:.2f}(s{gb['resp_dist']['seed']})] "
                    f"D90:{avg_dist_90:.2f}[G:{gb['avgdist90']['val']:.2f}(s{gb['avgdist90']['seed']})]cm | "
                    f"{pi_str} | σ {sig_str} | "
    ...
    # ── Guardar history do seed com melhor resp_dist ───────────────
    if gb['resp_dist']['seed'] == SEED:
        global_best_history    = {k: list(v) if isinstance(v, list) else v
                                   for k, v in history.items()}
        global_best_stopped_ep = _stopped_epoch

    elapsed = (time.time() - start) / 60
    log_print(f"  ✅ Seed {SEED} concluído | epochs={_stopped_epoch} | {elapsed:.1f}min")
    log_print(f"     Globais actuais: "
              f"RespD={gb['resp_dist']['val']:.2f}cm(s{gb['resp_dist']['seed']}) | "
              f"D90={gb['avgdist90']['val']:.2f}cm(s{gb['avgdist90']['seed']}) | "
              f"NLL={gb['nll']['val']:.3f}(s{gb['nll']['seed']})")

# ==========================================
# RESTAURAR — cada critério usa o seu melhor
# ==========================================
log_print(f"\n{'='*60}")
log_print(f"🏆 RESULTADOS FINAIS (por critério, cross-seed):")
for key, info in gb.items():
    log_print(f"   {key:>15}: {info['val']:.4f}  (seed {info['seed']})")
log_print(f"   Tempo total: {(time.time()-start)/60:.1f} min")
log_print(f"{'='*60}")

# Variáveis para o resto do código (avaliação usa melhor resp_dist)
SEED            = gb['resp_dist']['seed']
history         = global_best_history
_stopped_epoch  = global_best_stopped_ep

best_resp_dist  = gb['resp_dist']['val'];       best_resp_weights           = gb['resp_dist']['w']
best_resp_mean  = gb['resp_mean']['val'];        best_resp_mean_weights      = gb['resp_mean']['w']
best_resp90     = gb['resp90']['val'];           best_resp90_weights         = gb['resp90']['w']
best_nll        = gb['nll']['val'];              best_nll_weights            = gb['nll']['w']
best_avgdist_mean   = gb['avgdist_mean']['val']; best_avgdist_mean_weights   = gb['avgdist_mean']['w']
best_avgdist_median = gb['avgdist_median']['val'];best_avgdist_median_weights = gb['avgdist_median']['w']
best_avgdist90      = gb['avgdist90']['val'];    best_avgdist90_weights      = gb['avgdist90']['w']
best_combined90     = gb['combined90']['val'];   best_combined90_weights     = gb['combined90']['w']
best_weights        = best_resp_weights   # modelo principal para avaliação
...
_avgdist_criteria = [
    ("AvgDist — Média",
     best_avgdist_mean_weights,
     f"Best={best_avgdist_mean:.2f}cm  (seed {gb['avgdist_mean']['seed']})"),
    ("AvgDist — Mediana",
     best_avgdist_median_weights,
     f"Best={best_avgdist_median:.2f}cm  (seed {gb['avgdist_median']['seed']})"),
    ("AvgDist — 90% menor",
     best_avgdist90_weights,
     f"Best={best_avgdist90:.2f}cm  (seed {gb['avgdist90']['seed']})"),
]
...
plt.suptitle(
    f'{KEYWORD} — Seleção por AvgDist (Argmax)\n'
    f'(elipses: 1σ sólido · 2σ tracejado | ★ = μ expert | seed por painel)',
    fontsize=12
)
plt.tight_layout()
...
_resp_criteria = [
    ("RespDist — Média",
     best_resp_mean_weights,
     f"Best={best_resp_mean:.2f}cm  (seed {gb['resp_mean']['seed']})"),
    ("RespDist — Mediana",
     best_resp_weights,
     f"Best={best_resp_dist:.2f}cm  (seed {gb['resp_dist']['seed']})"),
    ("RespDist — 90% menor",
     best_resp90_weights,
     f"Best={best_resp90:.2f}cm  (seed {gb['resp90']['seed']})"),
]
...
plt.suptitle(
    f'{KEYWORD} — Seleção por RespDist\n'
    f'(elipses: 1σ sólido · 2σ tracejado | ★ = μ expert | seed por painel)',
    fontsize=12
)
plt.tight_layout()
